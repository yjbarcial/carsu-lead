<template>
    ...
    <div class="token-card">
      ...
      <div class="field-group">
        ...
        <div class="email-prefix-wrapper" :class="{ error: emailError }">
          <input type="text" v-model="supervisorEmailPrefix" class="email-prefix-input" placeholder="yourname" />
          <span class="email-suffix">@carsu.edu.ph</span>
        </div>
      </div>

      <p v-if="tokenError" class="token-error">{{ tokenError }}</p>
      <button class="btn-load" @click="loadByToken" :disabled="isLoading">Verify &amp; Load IDP</button>
    </div>
  </div>
  ...
  <div v-if="screen === 'review'">
    ...
    <div class="container">
      ...
      <div class="page-title">
        ...
        <p>Supervisor's Review — Read-only view of employee submission. Complete Section IV below.</p>
        ...
        <div class="section-body">
          <div class="ro-grid ro-grid-2">
            ...
            <div class="ro-field">
              ...
              <div class="ro-value">{{ idpData.educAttainment || '—' }}{{ idpData.educAttainmentSpec ? ' — ' + idpData.educAttainmentSpec : '' }}</div>
            </div>
            <div class="ro-field">
            ...
            <table class="ro-table">
              ...
              <tbody>
                ...
                <tr v-for="(row, i) in competencyRows" :key="i">
                  ...
                  <td><span class="cluster-badge" :class="clusterClass(row.competencyGroup)">{{ row.competencyGroup || '—' }}</span></td>
      ...
      <div class="section-card">
        <div class="section-header">
          ...
          <div><h3>Professional Advancement through Capacity-Building and Trainings (Pro-ACT)</h3></div>
          <div class="readonly-badge">Read-only</div>
        </div>
        <div class="section-body">
          <div class="table-wrapper">
      ...
      <div class="section-card">
        <div class="section-header">
          ...
          <div>
            ...
            <p>Complete all fields below — this section is for the supervisor only</p>
          </div>
        </div>
        <div class="section-body">
          <table class="assessment-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="guide-q">Are there any gaps in the employee's current performance that may be addressed through training, coaching, or mentoring?</td>
                <td class="response-cell">
                  <div class="checkbox-group" style="margin-bottom:12px">
                    <label class="checkbox-item" :class="{ checked: assessment.perfGapsYN === 'Yes' }">
                      <input type="radio" v-model="assessment.perfGapsYN" value="Yes" /> Yes
                    </label>
                    <label class="checkbox-item" :class="{ checked: assessment.perfGapsYN === 'No' }">
                      <input type="radio" v-model="assessment.perfGapsYN" value="No" /> No
                    </label>
                  </div>
                  <div v-if="assessment.perfGapsYN === 'Yes'" style="margin-top:8px">
                    <label class="field-label">If Yes, specify:</label>
                    <textarea v-model="assessment.perfGapsSpec" rows="3" placeholder="Describe the performance gaps…"></textarea>
                  </div>
                </td>
              </tr>
              <!-- 2. Readiness -->
              <tr>
                ...
                <td class="guide-q">Based on your observation, is the employee ready for higher responsibilities or academic growth (e.g., scholarship, promotion readiness)?</td>
                <td class="response-cell">
                  <div class="checkbox-group" style="margin-bottom:12px">
                    <label class="checkbox-item" :class="{ checked: assessment.readinessYN === 'Yes' }">
                      <input type="radio" v-model="assessment.readinessYN" value="Yes" /> Yes
                    </label>
                    <label class="checkbox-item" :class="{ checked: assessment.readinessYN === 'No' }">
                      <input type="radio" v-model="assessment.readinessYN" value="No" /> No
                    </label>
                  </div>
                  <label class="field-label">Remarks:</label>
                  <textarea v-model="assessment.readinessRemarks" rows="3" placeholder="Your remarks on readiness…"></textarea>
                </td>
              </tr>
              <!-- 3. Recommended interventions -->
              <tr>
                <td class="area-label">3. Recommended L&amp;D Interventions</td>
                <td class="guide-q">What specific learning and development interventions do you recommend? <em>(Check all applicable)</em></td>
                <td class="response-cell">
                  <div class="checkbox-group">
                    <label v-for="opt in interventionOptions" :key="opt" class="checkbox-item" :class="{ checked: assessment.interventions.includes(opt) }">
                      <input type="checkbox" :value="opt" v-model="assessment.interventions" /> {{ opt }}
                    </label>
                  </div>
                  <div v-if="assessment.interventions.includes('Others')" style="margin-top:10px">
                    <input type="text" v-model="assessment.interventionOther" placeholder="Specify other intervention…" />
                  </div>
                </td>
              </tr>
              ...
              <tr>
                ...
                <td class="guide-q">When should the recommended interventions ideally be implemented? Select a year first, then the quarter or period.</td>
                <td class="response-cell">
                  <!-- Year dropdown -->
                  <div style="margin-bottom: 12px; display: flex; align-items: center; gap: 10px;">
                    <small class="impl-hint" style="margin-bottom:0; white-space:nowrap;">Year</small>
                    <select v-model="assessment.implYear" style="width: auto; min-width: 120px; margin-bottom: 0;">
                  ...
                  <div class="checkbox-group">
                    <label v-for="opt in periodOptions" :key="opt" class="checkbox-item" :class="{ checked: assessment.implementationPeriod === opt }">
                      <input type="radio" name="implPeriod" :value="opt" v-model="assessment.implementationPeriod" /> {{ opt }}
                    </label>
                  </div>
                </td>
              </tr>
              ...
              <tr>
                <td class="area-label">5. Additional Comments or Development Notes</td>
                <td class="guide-q">Any other suggestions or remarks to support the employee's development plan?</td>
                <td class="response-cell">
                  <textarea v-model="assessment.additionalComments" rows="4" placeholder="Your additional comments or development notes…"></textarea>
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <div class="cert-block">
            <p><em>I hereby certify that I have reviewed and assessed the Individual Development Plan (IDP) of the concerned personnel and confirm that the proposed goals and interventions are aligned with their role, performance needs, and the strategic direction of the office.</em></p>
      ...
      <div class="submit-area">
        <p>By submitting, you confirm that your assessment is complete and accurate. HR will be notified immediately.</p>
        <button class="btn-submit" @click="submitAssessment" :disabled="isLoading">Submit Assessment</button>
      </div>
    </div>
  </div>
  ...
  <div v-if="screen === 'success'" class="success-wrap">
    <div class="success-card">
      ...
      <p>Your assessment for this IDP has been recorded. HRMS has been notified that the IDP is now fully complete.</p>
...
const competencyRows = computed(() => {
  try { return JSON.parse(idpData.value.competencyRowsJson || '[]'); } catch { return []; }
});
const agapRows = computed(() => {
  try { return JSON.parse(idpData.value.agapRowsJson || '[]'); } catch { return []; }
});
const proactRows = computed(() => {
  try { return JSON.parse(idpData.value.proactRowsJson || '[]'); } catch { return []; }
});
...
const interventionOptions = [
  'Technical Training', 'Soft Skills', 'Leadership Development',
  'Coaching / Mentoring', 'Academic Advancement', 'Job Rotation / Cross-training', 'Others',
];
...
function clusterClass(group) {
  const map = { Core: 'cluster-core', Leadership: 'cluster-leadership', Organizational: 'cluster-org', Technical: 'cluster-tech' };
  return map[group] || '';
}
...
async function loadByToken() {
  ...
  if (!token) { tokenError.value = 'Please enter the Reference ID.'; return; }
  if (!supervisorEmailPrefix.value.trim()) { emailError.value = true; tokenError.value = 'Please enter your CarSU email.'; return; }
  ...
  try {
    ...
    if (!res.ok) { tokenError.value = 'IDP not found. Please check your Reference ID.'; return; }

    const data = await res.json();

    // Verify the supervisor email matches
    if (data.supervisorEmail && data.supervisorEmail.toLowerCase() !== email.toLowerCase()) {
      tokenError.value = 'Email does not match the supervisor on record for this IDP.';
      return;
    }
...
async function submitAssessment() {
  if (!assessment.perfGapsYN) { alert('Please indicate if there are performance gaps.'); return; }
  if (!assessment.readinessYN) { alert('Please indicate readiness for advancement.'); return; }
...
.token-logo img { width: 64px; height: 64px; object-fit: contain; }
.token-card h2 {
...
.nav-current { font-size: 12.5px; color: var(--gold); font-weight: 600; }
...
.section-header h3 { color: #fff; font-size: 15px; font-weight: 600; }
...
.ro-grid-2 { grid-template-columns: 1fr 1fr; }
...
.ro-table thead tr { background: var(--navy-mid, #1a5c1a); }
.ro-table thead th {
  padding: 10px 12px;
  color: #fff;
...
.ro-table thead th:last-child { border-right: none; }
.ro-table tbody tr { border-bottom: 1px solid var(--border); }
.ro-table tbody tr:nth-child(even) { background: #f9f8f5; }
.ro-table tbody td {
  padding: 10px 12px;
  vertical-align: top;
...
.ro-table tbody td:last-child { border-right: none; }
.empty-row td { text-align: center; color: var(--text-light); font-style: italic; }
.tc { text-align: center; font-weight: 600; color: var(--text-light); }
...
.checkbox-item.checked { border-color: var(--navy); background: rgba(0,51,0,0.05); }
.checkbox-item input[type="radio"],
.checkbox-item input[type="checkbox"] {
  width: auto; padding: 0; border: none;
  background: none; accent-color: var(--navy); cursor: pointer;
}
...
.area-label { font-weight: 600; color: #1a5c1a; font-size: 13px; min-width: 160px; }
.guide-q { color: var(--text-light); font-size: 13px; min-width: 220px; }
...
.cert-block p { font-size: 13px; color: var(--text-light); font-style: italic; margin-bottom: 16px; }
.sup-name {
...
.cert-block small { font-size: 11px; color: var(--text-light); display: block; margin-top: 4px; }
...
.btn-submit:hover { background: #1a5c1a; transform: translateY(-1px); }
...
.success-card > p { color: var(--text-light); font-size: 14px; margin-bottom: 20px; }
.notif-note {
...
.notif-note strong { color: var(--navy); font-size: 14px; }
.notif-note span { color: var(--text-light); font-size: 13px; }
...
.btn-home:hover { background: #1a5c1a; transform: translateY(-1px); }
...
@media (max-width: 640px) {
  ...
  .ro-field.span-2 { grid-column: span 1; }
  .section-body { padding: 24px 20px; }
  .token-card { padding: 32px 20px; }
}
</style>