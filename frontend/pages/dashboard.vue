  ...
  <div>
    ...
    <div v-if="screen === 'denied'" class="auth-screen">
      <div class="denied-card">
        ...
        <button class="btn-try" @click="screen = 'login'">
          ← Try Another Email
        </button>
      </div>
    </div>
    ...
    <div v-if="screen === 'dash'" class="dash-wrap">
      ...
      <div class="stats-row">
        ...
        >
          <div class="stat-icon">
            <svg viewBox="0 0 24 24" v-html="stat.icon"></svg>
          </div>
      ...
      <div v-if="activeTab === 'overview'" class="tab-panel active">
        <div class="charts-grid">
          ...
          <div class="chart-card">
            ...
            <div class="chart-wrap"><canvas ref="chartCampus"></canvas></div>
          </div>
          <div class="chart-card">
            <h4>Monthly Submissions</h4>
            <p class="chart-sub">IDP and LNA submissions over time</p>
            <div class="chart-wrap"><canvas ref="chartMonthly"></canvas></div>
          </div>
          <div class="chart-card">
            ...
            <div class="chart-wrap"><canvas ref="chartGaps"></canvas></div>
          </div>
        </div>
        <div
          class="charts-grid"
        ...
        >
          <div class="chart-card">
            ...
            <div class="chart-wrap"><canvas ref="chartPurpose"></canvas></div>
          </div>
          <div class="chart-card">
            <h4>Key Insights</h4>
            <p class="chart-sub">Aggregated highlights from all submissions</p>
      ...
      <!-- IDP SUBMISSIONS -->
      <div v-if="activeTab === 'idp'" class="tab-panel active">
        <div class="filter-bar">
          <input
            ...
            @input="renderIDP"
          />
          <select v-model="idpStatusFilter" @change="renderIDP">
            <option value="">All Statuses</option>
            <option value="PENDING">Pending</option>
            <option value="COMPLETE">Completed</option>
            <option value="OVERDUE">Overdue</option>
          </select>
          <select v-model="idpCampusFilter" @change="renderIDP">
            <option value="">All Campuses</option>
            <option v-for="c in idpCampuses" :key="c">{{ c }}</option>
          </select>
          <select v-model="idpYearFilter" @change="renderIDP">
            ...
            <thead>
              <tr>
                <th
                  ...
                  :key="col"
                  @click="sortTable('idp', i)"
                >
                  {{ col }}
                  <span class="sort-ind">{{ sortIndicator("idp", i) }}</span>
                </th>
                <th>Action</th>
              </tr>
            </thead>
      ...
      <!-- LNA SUBMISSIONS -->
      <div v-if="activeTab === 'lna'" class="tab-panel active">
        <div class="filter-bar">
          <input
            type="text"
            v-model="lnaSearch"
            ...
            @input="renderLNA"
          />
          <select v-model="lnaCampusFilter" @change="renderLNA">
            <option value="">All Campuses</option>
            <option>CSU Main Campus</option>
            <option>CSU Cabadbaran City Campus</option>
          </select>
          <select v-model="lnaYearFilter" @change="renderLNA">
            <option value="">All Years</option>
            ...
            <thead>
              <tr>
                <th
                  ...
                  :key="col"
                  @click="sortTable('lna', i)"
                >
                  {{ col }}
                  <span class="sort-ind">{{ sortIndicator("lna", i) }}</span>
                </th>
                <th>Action</th>
              </tr>
            </thead>
      ...
      <!-- HR REGISTRY -->
      <div v-if="activeTab === 'registry'" class="tab-panel active">
      ...
      <div class="modal">
        <div class="modal-header">
          ...
          <div class="modal-header-right">
            <button class="btn-pdf" @click="printModal('idp')">
              <svg viewBox="0 0 24 24">
                <path
    ...
    >
      <div class="modal">
        <div class="modal-header">
          ...
          <div class="modal-header-right">
            <button class="btn-pdf" @click="printModal('lna')">
              <svg viewBox="0 0 24 24">
                <path
                  d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"
                />
...
const idpCampuses = computed(() =>
  [...new Set(idps.value.map((r) => r.campus).filter(Boolean))].sort(),
);
...
const filteredLNAs = computed(() => {
  ...
  let rows = lnas.value.filter((r) => {
    ...
    return (
      ms &&
      (!lnaCampusFilter.value || r.campus === lnaCampusFilter.value) &&
...
const insights = computed(() => {
  ...
  lnas.value.forEach((r) => {
    try {
      if (
        safeJ(r.clusterSummaryRaw, []).some((c) => c.interventionNeeded === "Y")
      )
        flagCount++;
    } catch (e) {}
  });
...
async function doLogin() {
  ...
  if (!em) {
    loginHint.value = "Please enter your email.";
    loginHintType.value = "err";
    return;
  }
  if (!em.endsWith("@carsu.edu.ph")) {
    loginHint.value = "Must be a @carsu.edu.ph address.";
    loginHintType.value = "err";
    return;
  }
  loginHint.value = "Verifying...";
  loginHintType.value = "";
  loginLoading.value = true;
  try {
    const res = await fetch(
      `${API}/api/auth/check?email=${encodeURIComponent(em)}`,
    );
    const data = await res.json();
    if (data.authorized) {
      currentUser.value = { email: em, name: data.name };
      screen.value = "dash";
      loadDashboard();
    } else {
      screen.value = "denied";
    ...
    loginHint.value = "Network error. Try again.";
    loginHintType.value = "err";
  } finally {
...
async function loadDashboard() {
  ...
  try {
    ...
    const now = new Date();
    lastUpdated.value = `Last updated: ${now.toLocaleDateString("en-PH", {
      month: "short",
      day: "numeric",
      year: "numeric",
...
async function renderCharts() {
  ...
  const camps = [
    ...new Set(
      [
        ...idps.value.map((r) => r.campus),
        ...lnas.value.map((r) => r.campus),
      ].filter(Boolean),
    ),
  ];
  if (chartCampus.value)
    chartInstances.c = new Chart(chartCampus.value, {
      type: "bar",
      data: {
        labels: camps.map((c) => c.replace("CSU ", "").replace(" Campus", "")),
        datasets: [
          {
            label: "IDPs",
            data: camps.map(
              (c) => idps.value.filter((r) => r.campus === c).length,
            ),
          ...
          },
          {
            label: "LNAs",
            data: camps.map(
              (c) => lnas.value.filter((r) => r.campus === c).length,
            ),
  ...
  const mm = {};
  [
    ...idps.value.map((r) => ({ d: r.submittedAt, t: "i" })),
    ...lnas.value.map((r) => ({ d: r.submittedAt, t: "l" })),
  ].forEach(({ d, t }) => {
    if (!d) return;
    const dt = new Date(d);
    if (isNaN(dt)) return;
  ...
  if (chartMonthly.value)
    chartInstances.m = new Chart(chartMonthly.value, {
      type: "line",
      data: {
        labels: lbls.map((l) => {
          const [y, mo] = l.split("-");
  ...
  const sorted = Object.entries(freq)
    ...
    .slice(0, 8);
  if (chartGaps.value)
    chartInstances.g = new Chart(chartGaps.value, {
      type: "bar",
      data: {
        labels: sorted.map(([k]) => (k.length > 24 ? k.slice(0, 24) + "…" : k)),
        datasets: [
          {
            label: "# with gap",
            data: sorted.map(([, v]) => v),
  ...
  if (chartPurpose.value)
    chartInstances.p = new Chart(chartPurpose.value, {
      type: "pie",
      data: {
        labels: pLbls,
        datasets: [
...
function printModal(type) {
  const bodyEl = document.querySelector(
    type === "idp" ? ".modal-body" : ".modal-body",
  );
  const titleEl = document.querySelector(
    type === "idp" ? ".modal-header h3" : ".modal-header h3",
  );
  ...
  pa.innerHTML = `<div style="font-family:Georgia,serif;color:#1a1a2e">
    <div style="border-bottom:3px solid #1a4d2e;padding-bottom:10px;margin-bottom:18px">
      <div style="font-size:9px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#5a6070;margin-bottom:3px">Caraga State University — Office of Human Resource Management Services</div>
      <h1 style="font-size:18px;color:#1a4d2e;margin-bottom:2px;font-family:Georgia,serif">${
        titleEl?.textContent || ""
      }</h1>
    </div>
    ${bodyEl?.innerHTML || ""}
    <div style="margin-top:28px;padding-top:10px;border-top:1px solid #d8d4c8;font-size:9px;color:#aaa">Printed from CarSU HRMS LeaD Dashboard · ${new Date().toLocaleDateString(
      "en-PH",
...
function statusBadgeClass(s) {
  ...
  if (c === "OVERDUE") return "badge badge-red";
...
function statusBadgeLabel(s) {
  ...
  if (c === "PENDING") return "Pending Review";
...
<!-- Inline sub-components for modal detail views -->
<script>
// IDPDetail and LNADetail inline components are defined here
// In a real Nuxt project, split these into separate files under components/
const IDPDetail = {
  props: ["record"],
  template: `
    <div>
      <div class="m-section">
        ...
        <div class="dgrid g3">
          <div class="df"><label>Reference ID</label><code>{{ record.refId || '—' }}</code></div>
          <div class="df"><label>Year Covered</label><span>{{ record.yearCovered || '—' }}</span></div>
          <div class="df"><label>Purpose</label><span>{{ record.headerPurpose || '—' }}</span></div>
          <div class="df s3"><label>Name of Personnel</label><span>{{ record.employeeName || '—' }}</span></div>
          <div class="df"><label>Email</label><span>{{ record.email || '—' }}</span></div>
          <div class="df"><label>Position</label><span>{{ record.position || '—' }}</span></div>
          <div class="df"><label>Office / Unit</label><span>{{ record.office || '—' }}</span></div>
          <div class="df"><label>Campus</label><span>{{ record.campus || '—' }}</span></div>
          <div class="df"><label>Years in Position</label><span>{{ record.yearsInPosition || '—' }}</span></div>
          <div class="df"><label>Years in CSU</label><span>{{ record.yearsInCSU || '—' }}</span></div>
          <div class="df"><label>Supervisor</label><span>{{ record.supervisorName || '—' }}</span></div>
          <div class="df"><label>Supervisor Email</label><span>{{ record.supervisorEmail || '—' }}</span></div>
        </div>
      </div>
      <div class="m-section" v-if="record.competencyRows && record.competencyRows.length">
        <div class="m-section-title">Section I — Competency Assessment <span class="m-badge">Employee</span></div>
        <table class="m-table"><thead><tr><th>#</th><th>Target Competency</th><th>Current Level</th><th>Required Level</th><th>Suggested Interventions</th><th>Resource / Support</th><th>Timeline</th></tr></thead>
        <tbody><tr v-for="(row, i) in record.competencyRows" :key="i"><td>{{ i+1 }}</td><td>{{ row[1]||'—' }}</td><td>{{ row[2]||'—' }}</td><td>{{ row[3]||'—' }}</td><td>{{ row[4]||'—' }}</td><td>{{ row[5]||'—' }}</td><td>{{ row[6]||'—' }}</td></tr></tbody></table>
      </div>
      <div class="m-section">
        <div class="m-section-title">Section IV — Supervisor Assessment <span class="m-badge">Supervisor</span></div>
        <div v-if="record.status === 'COMPLETE' || record.supervisorReviewDate" class="dgrid">
          <div class="df"><label>Performance Gaps?</label><span>{{ record.perfGapsYN || '—' }}</span></div>
          <div class="df"><label>Readiness for Advancement?</label><span>{{ record.readinessYN || '—' }}</span></div>
          <div class="df s2"><label>Additional Comments</label><span style="white-space:pre-wrap">{{ record.additionalComments || '—' }}</span></div>
        </div>
        <div v-else style="background:rgba(245,195,0,.08);border:1px solid rgba(245,195,0,.35);border-radius:8px;padding:16px;font-size:13px;color:var(--navy)">
    ...
    <div>
      <div class="m-section">
        ...
        <div class="dgrid g3">
          <div class="df"><label>Reference ID</label><code>{{ record.refId || '—' }}</code></div>
          <div class="df"><label>Year Covered</label><span>{{ record.yearCovered || '—' }}</span></div>
          <div class="df"><label>Purpose</label><span>{{ record.purpose || '—' }}</span></div>
          <div class="df s3"><label>Office / Unit / College</label><span>{{ record.office || '—' }}</span></div>
          <div class="df"><label>Head of Office</label><span>{{ record.headOfUnit || '—' }}</span></div>
          <div class="df"><label>Position</label><span>{{ record.position || '—' }}</span></div>
          <div class="df"><label>Campus</label><span>{{ record.campus || '—' }}</span></div>
          <div class="df"><label>Total Personnel</label><span>{{ record.totalPersonnel || '—' }}</span></div>
          <div class="df"><label>Rater</label><span>{{ record.raterName || '—' }}</span></div>
        </div>
      </div>
    </div>
  `,
...
export default { components: { IDPDetail, LNADetail } };
</script>
